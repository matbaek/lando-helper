...
mt-composer() {
    ...
    echo "$MT_EMOJI ${MT_UNDERLINE}Available composer paths${MT_RESET}"
    ...
    local folder_paths=$(find $searchable_folder_paths -maxdepth 5 -name "composer.json" -type f -not -path "*/vendor/*" -not -path "*/node_modules/*" -not -path "*/modules/*" | sed 's|^\./||;s|/[^/]*$||')
    ...
    echo "$MT_EMOJI ${MT_BOLD}Running command:${MT_RESET} composer ${@}"
